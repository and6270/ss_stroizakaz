...
Процедура ВыполнитьКоманду() Экспорт
	ТекстДляФайла = СоздатьТекстCSV(, , Истина);
...
Функция ВыполнитьОбменССайтомКлиент(ТекстДляФайла)Экспорт
	// вместе с пользовотелем меняется "access"
	// + TODO дописать константу "access"
	// TODO дописать диалоговое окно сохранения в файл
	// TODO дописать справку при ошибке
	// Достууп сайта = "u;540097"
	// КодДоступаСайта = "egIBE3nvQCyPbzn6ryAbpi2pJyH4issWlJGrXQ-E";
	ДоступСайта = ПолучитьКонстанту("Расш6270_Доступ");
	КодДоступаСайта = ПолучитьКонстанту("Расш6270_КодДоступаСайта");
	// yJnTPjFCpzenO_PslA4hcbvXV9Dg-SMaLxmfD3PN
	// Вставить содержимое обработчика.
	//ТекстДляФайла = СоздатьТекстCSV(МассивДокументовУЦН, ВыгружатьНаименование, ВыгружатьДляЯндекса);
	// Сообщить(ТекстДляФайла);		
	...
	//АдреснаяСтрока = "/my/s3/data/file/editing.php?ver_id=1172010&access=u%3B540097&orderby=created&time=1634070860118";
	//( Черных обновлено 24.09.23
	//АдреснаяСтрока = "/my/s3/cms/v1/file/editing.php?ver_id=1172010&access=" + ДоступСайта + "&orderby=created&time=1634070860118";
	// )Черных
	//( Черных обновлено 10.07.24
	...
	// Найдем ид нашго файла, или для тестов подойдет и 
	//( Черных обновлено 24.09.23
	//Запрос = Новый HTTPЗапрос("/my/s3/data/file/?type_id=2&ver_id=1172010&access=" + ДоступСайта + "&rnd=7152",Заголовки);
	...
	//Запрос = Новый HTTPЗапрос("/my/s3/data/shop2/import/index.php?ver_id=1172010&access=u%3B540097&popup=1&shop_id=1468641&popup=1&rnd=2814&xhr=1&mode=queue&file_id=" + ИдФайла + "&from_shop2_import=1", ЗаголовкиИмпорт);
	//Запрос = Новый HTTPЗапрос("/my/s3/data/shop2/import/products/?ver_id=1172010&access=" + ДоступСайта + "&shop_id=1468641&popup=1&rnd=2814&xhr=1&file_id=" + ИдФайла + "&from_shop2_import=1&act=queue", ЗаголовкиИмпорт);
	//( Черных обновлено 24.09.23
	...
	//Если УведомлениеВТелеграм Тогда
	//	ЧатИД = "-1001574464419"; 
	//	НомераДокУстановкиЦен = ПолучитьСтрокуНомеровДокументов(МассивДокументовУЦН);
	//	ТекстСообщенияТГ = "Выгружены на сайт цены по документам №: " + НомераДокУстановкиЦен + ", "+ ПользователиКлиентСервер.ТекущийПользователь();
	//	Расш6270_Действия.ОтправитьТекстовоеСообщение("5107708327:AAHXdV92-eb3gHvusmczJBoZn0niADfu9_A", ЧатИД, ТекстСообщенияТГ);
	//КонецЕсли;
	Возврат Истина;
КонецФункции
...
Функция СоздатьТекстCSV(МассивДокументовУЦН, ВыгружатьНаименование=Ложь, ВыгружатьОстатки=Ложь) Экспорт
	...
	Если ВыгружатьОстатки Тогда
		Запрос.Текст =
			...
			|	ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних1.Цена, 0) КАК Цена2
			|ИЗ
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(, ВидЦены = &Цена2) КАК ЦеныНоменклатурыСрезПоследних1
			...
			|	И НоменклатураСпр.ВидНоменклатуры В ИЕРАРХИИ(&ГруппаВидНоменклатуры)";	
			...
			ГруппаВидНоменклатуры = Справочники.ВидыНоменклатуры.НайтиПоНаименованию("2. ИЗОЛЯЦИОННЫЕ МАТЕРИАЛЫ",Истина);
			Запрос.УстановитьПараметр("ГруппаВидНоменклатуры", ГруппаВидНоменклатуры);
	Иначе 
		Запрос.Текст = 
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|			ИНАЧЕ 0
			|		КОНЕЦ) КАК Цена,
			|	МАКСИМУМ(100000) КАК Количество,
			...
			|ГДЕ
			|	УстановкаЦенНоменклатурыТовары.ВидЦены В(&Цены) и УстановкаЦенНоменклатурыТовары.Ссылка в (&МассивСсылок)
			|
			|СГРУППИРОВАТЬ ПО
	...
	Если НЕ ВыгружатьОстатки Тогда
		Запрос = Новый Запрос;
...
Функция СоздатьОбновитьЗаказыИзФайла(ИмяФайла)
	...
	Пока Выборка.Следующий() Цикл
		...
		НовыйЗаказКлиента.БанковскийСчет = Неопределено;
		...
		ДопРеквизит.Значение = Выборка.Имя + ", " + Выборка.Телефон;
