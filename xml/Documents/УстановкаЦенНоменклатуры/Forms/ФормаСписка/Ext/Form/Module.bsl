...
Процедура ВыполнитьОбменССайтомКлиент(ВыгружатьНаименованиеНоменклатуры = Ложь)
	...
	ТекстCSV = Расш6270_МегаГрупп.СоздатьТекстCSV(МассивДокументовУЦН, ВыгружатьНаименованиеНоменклатуры, Ложь);
...
Функция СформироватьТекстНасервере(МассивДокумнтов) 
	ТекстCSV = Расш6270_МегаГрупп.СоздатьТекстCSV(МассивДокумнтов, Ложь, Ложь);	
	Возврат ТекстCSV;
КонецФункции
