...
Процедура ЗаполнитьЗаказыНаСервере()
	Заказы = Новый Массив;
	ОтборПоСтройке = Ложь;
	Если НЕ Объект.Стройка.Пустая() Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	ЗаказКлиентаДополнительныеРеквизиты.Значение = &Стройка";
		
		Запрос.УстановитьПараметр("Стройка", Объект.Стройка);
		
		Заказы = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку(0);
		ОтборПоСтройке = Истина;
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	И ВЫБОР
		|			КОГДА &ОтборПоСтройке
		|				ТОГДА ЗаказКлиента.Ссылка В (&Заказы)
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ";
	...
	Запрос.УстановитьПараметр("ОтборПоСтройке", ОтборПоСтройке);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	НовыйПараметр = Новый ПараметрВыбора("Отбор.Владелец", ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя", "Объект_239d2934c72249828c6c990c081fbf0d"));
	НовыйМассив = Новый Массив;
	НовыйМассив.Добавить(НовыйПараметр);
	Элементы.Стройка.ПараметрыВыбора = Новый ФиксированныйМассив(НовыйМассив);
КонецПроцедуры
